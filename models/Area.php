...
class Area {
    ...
    public function obtenerTodas() {
        ...
        $sql = "
            ...
            FROM areas a
            LEFT JOIN login l ON a.id = l.area_id
            GROUP BY a.id
    ...
    public function obtenerTodasActivas() {
        ...
        if ($result) {
            ...
            // DEBUG: Verificar error en la consulta
    ...
    public function crear($nombre, $descripcion) {
        ...
        $check = mysqli_query($this->conn, "SELECT * FROM areas WHERE nombre = '$nombre'");
    ...
    public function actualizar($id, $nombre, $descripcion, $nombre_anterior) {
        ...
        if (mysqli_query($this->conn, $sql)) {
            mysqli_query($this->conn, "UPDATE login SET area_id = '$id' 
                                       WHERE area_id IN (SELECT id FROM areas WHERE nombre = '$nombre_anterior')");
    ...
    public function eliminar($id) {
        $id = mysqli_real_escape_string($this->conn, $id);
        $check = mysqli_query($this->conn, "SELECT * FROM login WHERE area_id = '$id'");
        if (mysqli_num_rows($check) > 0) {
            return "No se puede eliminar el área porque está asignada a usuarios";
        }
        return mysqli_query($this->conn, "DELETE FROM areas WHERE id = '$id'")
            ? "Área eliminada exitosamente"
            : "Error al eliminar área: " . mysqli_error($this->conn);
    }
}
